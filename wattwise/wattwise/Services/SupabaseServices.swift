...
final class SupabaseNECService: NECServiceProtocol {
    func search(query: String, stateCode: String?, editionOverride: String?) async throws -> [NECSearchResult] {
        struct Request: Encodable {
            ...
            let edition_override: String?
        }
        struct Response: Decodable {
            let results: [NECSearchResult]
            let edition: String?
        }
        let r = try await APIClient.shared.post(
            endpoint: "nec_search",
            body: Request(query: query, state_code: stateCode, edition_override: editionOverride),
            responseType: Response.self
        )
        return r.results
    }
    ...
    func amendments(article: String, jurisdictionCode: String) async throws -> NECAmendmentsResult {
        struct Request: Encodable {
            let nec_article: String
            let jurisdiction_code: String
        }
        struct AmendmentDTO: Decodable {
            let id: UUID
            let type: String
            let summary: String
        ...
        struct Response: Decodable {
            let jurisdiction_code: String
            let adopted_edition: String
            let adoption_notes: String?
            let amendments: [AmendmentDTO]
        }
        let r = try await APIClient.shared.post(
            endpoint: "nec_amendments",
            body: Request(nec_article: article, jurisdiction_code: jurisdictionCode),
            responseType: Response.self
        )
        return NECAmendmentsResult(
            jurisdictionCode: r.jurisdiction_code,
            adoptedEdition: r.adopted_edition,
            adoptionNotes: r.adoption_notes,
            amendments: r.amendments.map {
                NECStateAmendment(id: $0.id, type: $0.type, summary: $0.summary,
                                  effectiveDate: $0.effectiveDate, source: $0.source)
            }
        )
    }
}
